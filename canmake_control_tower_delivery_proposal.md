...
An initial proof of concept has validated the core planning approach for CanMake Control Tower. The POC is not a production system; it was used to confirm the feasibility of the planning logic and reduce delivery risk for Phase 1.
...
Validated planning logic:

```text
Demand + BOM + Inventory -> Feasibility -> Shortages -> Production Plan
```

Phase 1 will convert this validated POC logic into production-ready modules that are modular, optimized, reusable, auditable, and suitable for multi-user business operations.

## 5. Production Readiness Gaps

The POC validates the planning direction, but the production version needs enterprise readiness.

The main production readiness gaps are:
...
- Add allocation, reservation, scheduling, progress tracking, supplier intelligence, forecasting, risk, and anomaly modules across later phases.

## 6. Target Production State
...
At a business level, Phase 1 will deliver the core production cockpit:
...
After Phase 1, the platform will expand into allocation, scheduling, supplier intelligence, forecasting, risk, anomaly detection, and advanced AI/ML modules.

### Phase 1 Enterprise Launch and Data Integration
...
This replaces the manual Excel upload workflow for the production system. Excel can still be used as a temporary support or testing format if needed, but it is not the intended Phase 1 production workflow.

## 7. Phase Roadmap

### Phase 1: Core Manufacturability Platform

Objective:

Convert the validated POC manufacturability logic into a production-ready Phase 1 workflow using enterprise planning data instead of manual workbook upload.
...
### Phase 2: Shortage Optimization and RM Allocation
...
### Phase 3: Scheduling, Capacity, and Progress Tracking
...
### Phase 4: Supplier Lead-Time and Procurement Intelligence
...
### Phase 5: Advanced Analytics and AI Modules
...
## 8. Phase 1 Project Timeline

Estimated start date: Monday, 22 Jun 2026. Week 1 begins on that date. The expected Phase 1 elapsed timeline is **3 to 4 months** using a parallel workstream delivery plan.

```mermaid
gantt
    title CanMake Control Tower - Phase 1 Project Timeline
    ...
    Phase 1 Production Core Ready            :milestone, m1, after p7, 0d
```
...
## 9. Accelerated Phase 1 MVP Option

If the team limits Phase 1 to only core manufacturability, material shortages, scenario run, secure enterprise launch, required screens, and private-server deployment, the first production MVP may be delivered in:
...
```mermaid
gantt
    title CanMake Control Tower - Phase 1 Faster MVP Option
    dateFormat  YYYY-MM-DD
    axisFormat  %b %d
...
## 10. Phase Dependency Overview

```mermaid
flowchart LR
    POC["POC validated planning flow<br/>Demand + BOM + Inventory"] --> R1["Phase 1<br/>Production Core"]
    R1 --> R2["Phase 2<br/>Shortage Optimization<br/>RM Allocation"]
    R2 --> R3["Phase 3<br/>Scheduling, Capacity<br/>Progress Tracking"]
    R3 --> R4["Phase 4<br/>Supplier Lead-Time<br/>Procurement Intelligence"]
    R4 --> R5["Phase 5<br/>Advanced Analytics<br/>AI Modules"]
```

Estimated roadmap for the full C&S scope across all phases:

```text
6 to 9 months
```

## 11. Phase 1 Delivery Workstreams and Estimated Timeline

The timeline below assumes structured C&S data is made available on time and the initial objective is a production-ready Phase 1 platform for core manufacturability analysis. The duration column shows each workstream inside the 3 to 4 month Phase 1 delivery window, with overlapping execution where practical.

| Workstream | Duration Within Phase 1 | Main Activities | Key Deliverables |
| --- | ---: | --- | --- |
| Workstream 0 - Discovery and Data Contract | Week 1 to 2 | Confirm Phase 1 scope, review available data, define input/output expectations, confirm Oracle APEX launch flow, confirm user roles and deployment constraints | Final data contract, launch assumptions, delivery assumptions, Phase 1 scope sign-off |
| Workstream 1 - Product and Delivery Foundation | Week 2 to 4 | Product branding, application foundation, environment setup, logging, health checks, initial deployment preparation | Branded application foundation, initial configuration, base delivery structure |
| Workstream 2 - Enterprise Data Integration | Week 4 to 8 | Read approved planning data from the C&S Oracle database, validate required fields, convert data into planning input format, create user-readable data quality messages | Data integration module, validation module, data readiness checks, test cases |
| Workstream 3 - Core Planning Workflow | Week 7 to 10 | Integrate manufacturability engine, add scenario creation and run workflow, store inputs and outputs, generate planning results and reports | Scenario workflow, planning results, result history, report downloads, core test suite |
| Workstream 4 - Planning Dashboard | Week 7 to 12 | Build demand overview, FG manufacturability, material shortage, production plan, scenario simulation, and report download screens | Production-ready planning dashboards, user workflow, reporting experience |
| Workstream 5 - Secure Enterprise Launch | Week 10 to 13 | Enable Oracle APEX button/menu launch for authorized users, support seamless launch without second login, session handling, logout/session expiry, login audit trail | Secure launch flow, secure access handling, audit trail |
| Workstream 6 - Multi-User Processing | Week 11 to 14 | Add background processing, job status tracking, scenario progress states, retry/failure handling, concurrency testing | Multi-user execution, job tracking, initial performance report |
| Workstream 7 - Production Readiness and UAT | Week 13 to 16 | Configure private-server deployment, service operation, logs, backup expectations, UAT deployment and issue closure | Production deployment, operations guide, deployment guide, UAT sign-off package |
...
The first table below shows recommended staffing for Phase 1. FTE means full-time equivalent, where `1.0` means full-time, `0.5` means half-time, and `0.2` means advisory or part-time support.
...
| Data Scientist / AI Engineer | 0.2 FTE in Phase 1 | 4 to 6 weeks in Phase 1, larger role in later phases | Validate planning logic, support planner assistant, prepare future forecasting/risk/anomaly modules |
...
| DevOps Engineer | 0.3 FTE during build, 0.7 FTE during production readiness | 4 to 6 weeks total effort across project | Private-server setup, deployment readiness, logs, service operation, rollout support |
...
Phase 1 team allocation by workstream:

| Workstream | Primary Team Allocation |
| --- | --- |
...
Data Scientist / AI Engineer allocation by phase:

| Phase | Recommended Allocation | Focus |
| --- | ---: | --- |
| Phase 1 - Production Core | 0.2 FTE | Planning logic validation and controlled planner assistant support |
| Phase 2 - Procurement and Allocation Intelligence | 0.2 FTE | Optimization support for shortage and allocation recommendations |
| Phase 3 - Scheduling and Production Execution | 0.2 FTE | Scheduling logic support and production planning analytics |
| Phase 4 - Supplier and Procurement Risk | 0.5 FTE | Supplier delay risk, lead-time variance, and procurement intelligence |
| Phase 5 - Advanced AI/ML | 1.0 FTE | Forecasting, risk scoring, anomaly detection, and AI planner assistant |

Recommended Phase 1 core team:
...
## 13. Delivery Priority Matrix

| Module | POC Validation Status | Production Need | Recommended Priority |
| --- | --- | --- | --- |
| FG Manufacturability Analysis | Validated in POC | Modular, optimized, reusable production workflow with enterprise data input and scenario history | Phase 1 |
| Material Shortage Analysis | Validated in POC | Modular shortage engine, reports, dashboards, and reusable result model | Phase 1 |
| Production Plan Recommendation | Validated in POC | Production-grade planning workflow, result persistence, and user interface | Phase 1 |
| Minimal Purchase Recommendation | Partially explored in POC | Optimization module and procurement recommendation workflow | Phase 2 |
| RM Allocation and Freeze | Not covered in POC | Reservation logic and optional controlled write-back | Phase 2 |
| Advanced Scheduling | Not covered in POC | Routing, capacity, calendar, sequencing logic | Phase 3 |
| Production Progress Tracking | Not covered in POC | WIP and work order status integration | Phase 3 |
| Supplier Lead-Time Management | Not covered in POC | Purchase and supplier history analytics | Phase 4 |
| Integration and Data Sync | POC used workbook input only | Direct enterprise data integration and controlled updates | Phase 1 onward |
| Forecasting | Not covered in POC | Historical demand model | Phase 5 |
| Risk Analysis | Not covered in POC | Risk model and business rules | Phase 5 |
| Anomaly Detection | Not covered in POC | Historical trend and anomaly model | Phase 5 |

## 14. Delivery Workstreams
...
| DevOps and Deployment | Private-server setup, service operation, logs, backups, production deployment, operations documentation |
...
| Controlled write-back permissions not approved | Allocation freeze cannot be automated in Phase 2 | Start with recommendation mode, add write-back after approval. |
| Heavy concurrent scenario runs | Slow user experience | Include multi-user processing and performance testing in Phase 1. |
| Scheduling rules are complex | Phase 3 may need longer validation | Deliver core feasibility first, then add scheduling in a controlled phase. |
...
The recommended strategy is to deliver CanMake Control Tower in controlled phases:

1. **Phase 1: Production Core**
...
The first production phase should focus on making that validated logic modular, optimized, reusable, and production-ready, covering FG manufacturability, material shortages, production plan recommendations, scenario history, report downloads, secure enterprise launch, multi-user processing, and private-server readiness. Later phases should add RM allocation/freeze controls, minimal purchase optimization, advanced scheduling, production progress tracking, supplier lead-time intelligence, forecasting, risk analysis, and anomaly detection.
